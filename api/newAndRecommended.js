...
const getNewReleases = async () => {
  return await httpClient.get(`/browse/new-releases?limit=50`, apiConfig(accessToken));
};
...
const getRecommendedTracks = async (topArtists) => {
  ...
  return (seeds.artists.length || seeds.tracks.length || seeds.genres.length) ?
    await httpClient.get(`/recommendations?limit=100&seed_artists=${seeds.artists}&seed_tracks=${seeds.tracks}&seed_genres=${seeds.genres}`, apiConfig(accessToken)) :
...
async function newAndRecommended(req, res){
  ...
  try{
    ...
    //combine data
    let allItems = Array.from(newReleases.data.albums.items);
    allItems.push.apply(allItems, [...recommendedTracks.data.tracks, ...recommendedArtists.data.artists]);
